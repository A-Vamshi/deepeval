...
## Pytest Integration For CI/CD Evals

In pull request workflows on GitHub Actions, `deepeval test run` posts a markdown summary of your evaluation results as a PR comment after pytest finishes.

Write your evals in a `test_*.py` file using `assert_test()`, `evaluate()`, or `evals_iterator()`.

### Example test file

```python title="tests/evals/test_cicd.py"
...
def test_correctness():
    ...
    test_cases = [
        LLMTestCase(
            ...
            actual_output="...",  # Replace with your LLM application's output
            expected_output="...",
        ),
        ...
    ]
...
:::note
The `evaluate()` and `evals_iterator()` methods do not raise errors when metrics fail, so a single `test_*` function can still pass in pytest even when scores are low. Pass the `--pass-rate` and `--required-metrics` flags on `deepeval test run` to fail the workflow after all tests finish if those thresholds are not met. To fail pytest immediately on a bad case, use `assert_test()`.
:::

### GitHub Actions workflow

```yaml title=".github/workflows/deepeval.yml"
  ...
  test:
    ...
    steps:
      ...
      - name: Run `deepeval` unit tests
        run: |
          poetry run deepeval test run tests/evals/test_cicd.py \
            --pass-rate 0.80 \
            --required-metrics Correctness
```

:::tip 
On **pull request** workflows, `deepeval test run` posts a markdown summary on the PR after pytest finishes — passing and failing cases, per-metric scores, and aggregates.

The `--pass-rate` and `--required-metrics` flags (shown in the command above) fail the workflow when overall pass rate or named metrics do not meet your bar, even if individual pytest tests passed. See [`--pass-rate`](/docs/evaluation-flags-and-configs#pass-rate-github-actions) and [`--required-metrics`](/docs/evaluation-flags-and-configs#required-metrics-github-actions) for details.
:::

## Pytest Integration For CI/CD Evals

Drop `deepeval test run` into a workflow to run your unit tests on every push or PR. Traced evals that use `evals_iterator()` inside a `test_*.py` file are supported the same way as `assert_test()` and `evaluate()` for GitHub Actions PR comments. This example uses `poetry` for installation and `OPENAI_API_KEY` as your LLM judge. Add `CONFIDENT_API_KEY` to send results to Confident AI.
...
On pull request workflows, results are posted as a PR comment. Add `--pass-rate` or `--required-metrics` when you need deployment gates beyond individual test pass/fail.
