...
:::note
`evaluate()` does not raise when metrics fail. Assert on `result.test_results` (as above) or use `assert_test()` if you want pytest to fail automatically on metric failures.
:::

### GitHub Actions workflow

Trigger on pull requests, grant comment permissions, and pass the required secrets. Use `--pass-rate` and `--required-metrics` to enforce thresholds in addition to posting the PR comment.
...
When a pull request is opened, this workflow runs your evals and comments on the PR with passing and failing test cases, per-metric scores, and aggregate metrics. Set `CONFIDENT_API_KEY` to include a link to the full run on Confident AI.

Use [`--pass-rate`](/docs/evaluation-flags-and-configs#pass-rate-github-actions) and [`--required-metrics`](/docs/evaluation-flags-and-configs#required-metrics-github-actions) to enforce deployment thresholds.
