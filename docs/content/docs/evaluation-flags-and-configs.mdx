...
### Pass Rate (GitHub Actions)

The `--pass-rate` flag sets the minimum fraction of test cases that must pass (e.g., `0.8` for 80%). On GitHub Actions, the job fails if the overall pass rate falls below this value.

```
deepeval test run test_example.py --pass-rate 0.8
```

### Required Metrics (GitHub Actions)

The `--required-metrics` flag accepts a comma-separated list of **metric class names** (e.g. `TaskCompletionMetric`, `GEval`) that must pass on every test case. Enforced on GitHub Actions alongside `--pass-rate`.

```
deepeval test run test_example.py --required-metrics "AnswerRelevancyMetric, ContextualPrecisionMetric"
```

### GitHub Actions PR Comments

On GitHub Actions pull request workflows, `deepeval test run` posts a markdown evaluation report as a PR comment when the run completes.
...
- `CONFIDENT_API_KEY` (optional) to link to the full run on Confident AI
...
env:
  GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
run: poetry run deepeval test run test_example.py --pass-rate 0.8 --required-metrics AnswerRelevancyMetric
```
