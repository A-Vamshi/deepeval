...
- **Evaluate the complete ordered [agent trajectory](/docs/evaluation-trajectory-based-llm-evals)** across its agent, LLM, and tool spans.
...
These exact pairs were tested locally against the source tree, versioned `4.2.1`,
including the unreleased compatibility fix. The regression tests run as part of the
[existing Pydantic AI CI job](https://github.com/confident-ai/deepeval/blob/main/.github/workflows/test_integrations.yml),
which installs the latest Pydantic AI release rather than pinning these pairs.
The published `4.2.1` package does not include the fix for Pydantic AI 2's tool-span attributes.
:::

| Pydantic AI version | Compatible `deepeval` versions |
| --- | --- |
| `1.94.0` | `4.2.1` + unreleased compatibility fix |
| `1.95.0` | `4.2.1` + unreleased compatibility fix |
| `1.107.4` | `4.2.1` + unreleased compatibility fix |
| `2.40.0` | `4.2.1` + unreleased compatibility fix |

Tests cover synchronous, asynchronous, and streaming runs, agent/LLM/tool spans,
tool arguments and results, and trace context cleanup without external API calls.
Historical mappings for earlier DeepEval releases are not established here.

Pydantic AI **1.95.0** introduced `capabilities=[Instrumentation(settings=...)]`.
Pydantic AI **2.0 removed** the `Agent(instrument=...)` constructor keyword;
`DeepEvalInstrumentationSettings` is still the settings object. The examples below
use the capability API. On older Pydantic AI 1.x, use
`Agent(..., instrument=DeepEvalInstrumentationSettings())`.

Pydantic AI 2 also defaults to telemetry format version 5. The compatibility fix
reads its `gen_ai.tool.call.arguments` and `gen_ai.tool.call.result` attributes
while retaining support for the old names. Telemetry format versions are separate
from Pydantic AI package versions. See the
[Pydantic AI migration guide](https://ai.pydantic.dev/migration/).
...
```bash
pip install -U deepeval "pydantic-ai-slim[openai]>=1.95,<3" opentelemetry-sdk opentelemetry-exporter-otlp-proto-http
```
...
Pass `DeepEvalInstrumentationSettings` to Pydantic AI's `Instrumentation` capability in the `Agent`'s `capabilities` list. From that point on, any `agent.run(...)`, `agent.run_sync(...)`, or `agent.run_stream(...)` call produces a trace `deepeval` can read.
...
agent = Agent(
    ...
    system_prompt="Be concise, reply with one sentence.",
    capabilities=[
        Instrumentation(settings=DeepEvalInstrumentationSettings(name="my-agent"))
    ],
)
...
Trajectory evaluation scores the complete ordered trace: every agent decision, LLM call, and tool interaction in sequence across the applicable spans. This is distinct from output-only evaluation, which scores only the final response, and individual component evaluation, which scores one span at a time.
...
See the full [trajectory evaluation guide](/docs/evaluation-trajectory-based-llm-evals) for metric details and trace requirements. Next, apply metrics to individual components when you need span-level diagnostics.
    ...
    },
    {
      ...
      answer: (
        <>
          ...
          <code>capabilities=[Instrumentation(settings=DeepEvalInstrumentationSettings())]</code>, run it in a
